...
await axios.get('https://backend-movielist.onrender.com/').then(response=>setData([response.data[0].results, response.data[1].results,response.data[2].results]))}}, []);
...
{details[0]?
   <div style={details[0].length<=0?{visibility:'hidden'}:{visibility:'visible'}} onClick={()=>setDetails([])} className='z-10 w-screen h-screen flex items-center justify-center bg-slate-700 fixed' >
...
<div   style={{"margin-left":`${threeLine}px`}} className='w-1/6  flex scroll overflow-x-hidden max-[640px]:overflow-x-scroll'>
   ...
   {data.length>1?data[2].map(e=><img onClick={()=>setDetails([e])} className='hover:bg-blue-500 cursor-pointer p-4 w-1/4 shadow-2xl  scrollbar-hide' src={"https://image.tmdb.org/t/p/original/"+e.poster_path} alt="" />):""}</div>
