...
export default function Series(){
   ...
   useEffect(() => { run()
   async function run(){
   await axios.get('https://movie-list-dayvison.herokuapp.com/series').then(response=>setData([response.data[0].results, response.data[1].results,response.data[2].results,response.data[3].results]))}}, []);  
   function genero(x){
...
<ScrollContainer className='flex  overflow-x-scroll'>{data.length>1?data[0].map(e=><img  onClick={()=>setDetails([e])} className='cursor-pointer w-1/4 shadow-2xl m-8 scrollbar-hide' src={"https://image.tmdb.org/t/p/original/"+e.poster_path} alt="" />):"carregando"}</ScrollContainer>

<h1 className='w-screen text-center'>filmes de ações</h1>
<ScrollContainer className='flex  overflow-x-scroll'>{data.length>1?data[1].map(e=><img  onClick={()=>setDetails([e])} className='cursor-pointer w-1/4 shadow-2xl m-8 scrollbar-hide' src={"https://image.tmdb.org/t/p/original/"+e.poster_path} alt="" />):"carregando"}</ScrollContainer>
<h1 className='w-screen text-center'>Filmes de aventuras</h1>
<ScrollContainer className='flex  overflow-x-scroll'>{data.length>1?data[2].map(e=><img  onClick={()=>setDetails([e])} className='cursor-pointer w-1/4 shadow-2xl m-8 scrollbar-hide' src={"https://image.tmdb.org/t/p/original/"+e.poster_path} alt="" />):"carregando"}</ScrollContainer>
<h1 className='w-screen text-center'>filmes de animações</h1>
<ScrollContainer className='flex  overflow-x-scroll'>{data.length>1?data[3].map(e=><img  onClick={()=>setDetails([e])} className='cursor-pointer w-1/4 shadow-2xl m-8 scrollbar-hide' src={"https://image.tmdb.org/t/p/original/"+e.poster_path} alt="" />):"carregando"}</ScrollContainer>
