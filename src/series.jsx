...
export default function Series(){
   ...
   useEffect(() => { run()
   async function run(){
   await axios.get('https://movies-88q6.onrender.com/series').then(response=>setData([response.data[0].results, response.data[1].results,response.data[2].results,response.data[3].results]))}}, []);  
   function genero(x){
